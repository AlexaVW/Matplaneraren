...
function getNutritions(ingredient) { 
    ...
    fetch(urlNutrition)
        ...
        .then(
            function (data) {
                console.log(data);
                createCard(ingredient, data); // The card is created after the nutritions are fetched
            }
        )
        .catch(
            function (error) {
...
// Search---------------------------------------------------------------
// Filtering the list with ingredients and showing the options in a dropdown
function getIngredientOptions() {
    ...
    ingredients.map(function (ingredient) { 
        if (ingredient.ingredientName.toLowerCase().includes(searchInput)) { 
            ...
            // When user clicks on an option from the dropdown
            option.onclick = function () { 
                searchText.value = ingredient.ingredientName; 
                selectedIngredient = ingredient; // Storing the selected ingredient
                ingredientOptions.innerHTML = ""; 
            }
...
function addIngredient() { 
    ...
    // Getting the nutritions from the selected ingredient, then creating the card
    getNutritions(selectedIngredient); 
}

// Create card for the ingredient--------------------------------------------
function createCard(ingredient, dataNutrition) {

    let nutrition = getSelectedNutrition(dataNutrition);
    ...
    let deleteButton = createDeleteButton(card, ingredientValues);
...
// createCard functions-----------------------------------------------------------------
function getSelectedNutrition(dataNutrition){
...
function createDeleteButton(card, ingredientValues){
